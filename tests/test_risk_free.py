...
from spx_trade_desk.core.rates import fetch_sgov_no_risk_yield, get_risk_free_rate


def test_fetch_sgov_no_risk_yield_uses_yfinance_yield_plus_expense_ratio():
    with patch("spx_trade_desk.core.rates.yf") as mock_yf:
...
def test_get_risk_free_rate_falls_back_to_default_when_fetch_fails():
    with patch("spx_trade_desk.core.rates.fetch_sgov_no_risk_yield", side_effect=Exception("network error")):
