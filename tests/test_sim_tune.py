...
"""Tests for the strategy-tuning variant runner (sim_tune.py)."""
import copy
import csv
import json
import os
...
def _clean():
    ...
    sim_jobs._STRATEGY_CACHE.clear()
    yield
    sim_jobs.reset_registry()
    sim_jobs._STRATEGY_CACHE.clear()
...
def test_apply_knobs_whitelist_and_unknown_raises():
    s = _strategy()
    sim_tune.apply_knobs(s, {"short_delta.min": 0.05, "entry_window.start": "9:45",
    ...
    sim_tune.apply_knobs(s, {"take_profit.pct": None, "credit.max": None,
    ...
    with pytest.raises(ValueError, match="allowed knobs"):
        sim_tune.apply_knobs(_strategy(), {"trend.period": 14})
    with pytest.raises(ValueError, match="vix_op"):
        sim_tune.apply_knobs(_strategy(), {"volatility.vix_op": "sideways"})


def test_apply_knobs_does_not_mutate_the_original():
    s = _strategy()
    orig = copy.deepcopy(s)
    sim_tune.apply_knobs(copy.deepcopy(s), {"short_delta.max": 0.50})
...
def test_unsupported_live_strategy_fails_before_compute(tmp_path):
    ...
    with pytest.raises(ValueError, match="trend"):
        sim_tune.run_experiment(_spec(strategy="TRENDY"), str(tmp_path / "out"),
...
def test_unknown_strategy_and_bad_spec_fail_clean(tmp_path):
    with pytest.raises(ValueError, match="unknown strategy"):
        sim_tune.run_experiment(_spec(strategy="Nope"), str(tmp_path),
                                strategies={"T": _strategy()}, log=_QUIET)
    with pytest.raises(ValueError, match="spec key"):
        sim_tune.run_experiment(_spec(bogus_key=1), str(tmp_path),
                                strategies={"T": _strategy()}, log=_QUIET)
    with pytest.raises(ValueError, match="reserved"):
        spec = _spec(variants=[{"name": "baseline", "knobs": {}}])
        sim_tune.run_experiment(spec, str(tmp_path),
                                strategies={"T": _strategy()}, log=_QUIET)
...
def test_runner_crn_identical_market_paths(tmp_path):
    ...
    results = sim_tune.run_experiment(_spec(), str(tmp_path),
                                      strategies={"T": _strategy()}, log=_QUIET)
...
def test_runner_is_deterministic_across_invocations(tmp_path):
    ...
    for out in (out1, out2):
        sim_tune.run_experiment(_spec(), str(out), strategies={"T": _strategy()},
                                log=_QUIET)
...
def test_runner_writes_csv_and_json_schemas(tmp_path):
    sim_tune.run_experiment(_spec(), str(tmp_path), strategies={"T": _strategy()},
                            log=_QUIET)
    with open(tmp_path / "results.csv", newline="", encoding="utf-8") as f:
        rows = list(csv.DictReader(f))
        assert rows[0].keys() == dict.fromkeys(sim_tune.CSV_COLUMNS).keys()
...
def test_runner_multi_seed_rows(tmp_path):
    results = sim_tune.run_experiment(_spec(), str(tmp_path), seeds=[42, 43],
                                      strategies={"T": _strategy()}, log=_QUIET)
...
def test_main_cli_smoke_end_to_end(tmp_path, monkeypatch):
    ...
    rc = sim_tune.main(["--spec", str(spec_path), "--out", str(tmp_path / "out"),
